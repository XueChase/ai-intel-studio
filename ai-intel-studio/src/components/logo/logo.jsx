...
export function Logo({ sx, disabled, className, href = '/', isSingle = true, ...other }) {
  ...
  const fullLogo = (
    <FullLogoWrap>
      <FullLogoIcon>{iconSvg}</FullLogoIcon>
      <Typography component="span" variant="subtitle2" noWrap sx={{ fontWeight: 700, color: 'text.primary' }}>
        {CONFIG.appName}
      </Typography>
    </FullLogoWrap>
  );
...
const FullLogoWrap = styled('span')(() => ({
  width: '100%',
  height: '100%',
  display: 'inline-flex',
  alignItems: 'center',
...
const FullLogoIcon = styled('span')(() => ({
  width: 40,
  height: 40,
  display: 'inline-flex',
  flexShrink: 0,
