...
`.trim();
    ...
    const headingOverrides = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']
    ...
    const renderer = {
        ...
        image(token) {
            ...
            if (options.legend === 'none')
                caption = '';
            else if (options.legend === 'title')
                caption = title;
            else if (options.legend === 'alt')
                caption = alt;
            else if (options.legend === 'title-alt')
                caption = title || alt;
            else
                caption = alt || title;
...
export function generatePureHTML(raw) {
    ...
    return plain.parse(normalizedMarkdown);
}
export function solveWeChatImage(root) {
...
async function getHljsStyles() {
    const hljsLink = document.querySelector('#hljs');
    if (!hljsLink)
        return '';
    try {
...
function getThemeStyles() {
    const themeStyle = document.querySelector('#md-theme');
...
function flattenInlineBreaksForWechat(root) {
...
function wrapTextAfterStrongForWechat(root) {
...
async function getStylesToAdd() {
    const themeStyles = getThemeStyles();
    const hljsStyles = await getHljsStyles();
    return [themeStyles, hljsStyles].filter(Boolean).join('');
}
export async function processClipboardContent(primaryColor, options) {
    ...
    clipboardDiv.innerHTML = clipboardDiv.innerHTML
        ...
        .replace(/var\(--md-font-family\)/g, options?.fontFamily
    ...
    clipboardDiv.innerHTML = clipboardDiv.innerHTML.replace(/<tspan([^>]*)>/g, '<tspan$1 style="fill: #333333 !important; color: #333333 !important; stroke: none !important;">');
